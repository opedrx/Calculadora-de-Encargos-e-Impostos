document.addEventListener('DOMContentLoaded', () => {
    ...
    const btnTabCLT = document.getElementById('btn-tab-clt');
    const btnTabPJ = document.getElementById('btn-tab-pj');
    ...
    function switchTab(mode) {
        if (mode === 'clt') {
            ...
            btnTabCLT.classList.add('active');
            btnTabPJ.classList.remove('active');
        } else {
            formCLT.style.display = 'none';
            formPJ.style.display = 'block';
            btnTabCLT.classList.remove('active');
            btnTabPJ.classList.add('active');
        }
        relatorioDiv.innerHTML = '';
    }

    btnTabCLT.addEventListener('click', () => switchTab('clt'));
    btnTabPJ.addEventListener('click', () => switchTab('pj'));
    ...
    function calcularINSS(salario) {
        ...
        for (const faixa of faixas) {
            ...
            let baseFaixa = Math.min(salarioRestante, faixa.teto - tetoAnterior);
    ...
    btnCalcularPJ.addEventListener('click', () => {
        ...
        if (regime === 'simples') {
            ...
            nomeRegime = "Simples Nacional (Anexo III - 6% Simplificado)";
        } else {
            resultadoPJ = calcularLucroPresumido(faturamento);
            nomeRegime = "Lucro Presumido (Serviços - 32% Simplificado)";
        }
